import pytest
import pathlib
from backend.app.kb.retriever import (
...
def test_no_orphan_files():
    ...
    for f in kb_files:
        assert f.name in refs, f"Orphan file found: {f.name}"

def test_no_dangling_references():
    kb_files = {f.name for f in get_all_kb_files()}
    refs = get_all_referenced_docs()
    for r in refs:
        assert r in kb_files, f"Dangling reference found: {r}"

def test_file_format_and_budget():
    kb_files = get_all_kb_files()
    for f in kb_files:
        ...
        assert h1_count == 1, f"File {f.name} has {h1_count} H1 headers, expected exactly 1."
        ...
        if f.name in new_files:
            assert "KẾT LUẬN CHO AI" in content, f"File {f.name} missing 'KẾT LUẬN CHO AI'"
        
        # 4. Không lộ nguồn: không doc nào chứa chuỗi `.md` trong phần thân.
        if f.name != "phong_cach_luan_giai.md":
            assert ".md" not in content, f"File {f.name} contains '.md' in content, which exposes source."
        
        # 5. Ngân sách: không doc nào vượt 60 dòng.
        assert len(lines) <= 60, f"File {f.name} exceeds 60 lines (has {len(lines)} lines)"
