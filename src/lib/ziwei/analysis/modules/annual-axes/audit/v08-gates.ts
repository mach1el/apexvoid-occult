...
function samplesToObservations(samples: V08HoldoutSample[]): AnnualAxesAuditObservation[] {
...
function vectorDist(vectors: number[][]) {
  const sds: number[] = [];
  const ranges: number[] = [];
  let allSixAbove50 = 0;
  let fiveOrMoreAbove50 = 0;
  let allSixInside45To65 = 0;
  let atLeastOneAtOrBelow40 = 0;
  ...
  for (const vals of vectors) {
    ...
    if (vals.some((v) => v <= 40)) atLeastOneAtOrBelow40 += 1;
    if (vals.some((v) => v >= 60)) atLeastOneAtOrAbove60 += 1;
    if (vals.some((v) => v <= 40) && vals.some((v) => v >= 60)) oneLowAndOneHigh += 1;
  ...
  return {
    ...
    atLeastOneAtOrBelow40Rate: rate(atLeastOneAtOrBelow40, total),
...
export function computeV08HoldoutMetrics(samples: V08HoldoutSample[]): Record<string, number> {
  const scores = samples.map((s) => s.score);
  const zs = samples.map((s) => s.effectiveZ);
  const scoreDist = scoreDistribution(scores);
  const observations = samplesToObservations(samples);
  const report = computeDistributionReport("annual-axes-v0.8", observations);
  ...
  const vec = vectorDist(vectors);
  const maxAbsCorr = Math.max(
    0,
    ...Object.values(report.interAxisCorrelation).map((v) => Math.abs(v)),
  );
  const tp4cMax = samples.reduce((m, s) => Math.max(m, Math.abs(s.tp4cSignedContribution)), 0);
  const boundaryRate = rate(
    scores.filter((v) => v === 5 || v === 95).length,
    scores.length,
  );
  const allFinite = samples.every(
    (s) =>
      Number.isFinite(s.score) &&
      Number.isFinite(s.effectiveZ) &&
      Number.isFinite(s.directSignedRaw),
  )
    ? 1
    : 0;
  const allInRange = scores.every((v) => v >= 5 && v <= 95) ? 1 : 0;

  const metrics: Record<string, number> = {
    allFinite,
    allScoresInRange: allInRange,
    unavailableRate: report.unavailableRate,
    boundaryScoreRate: boundaryRate,
    tp4cSignedContributionMaxAbs: tp4cMax,
    globalMedianScore: scoreDist.median,
    globalMeanScore: scoreDist.mean,
    positiveEffectiveZRate: rate(zs.filter((v) => v > 0).length, zs.length),
    negativeEffectiveZRate: rate(zs.filter((v) => v < 0).length, zs.length),
    ...
    atLeastOneAtOrBelow40Rate: vec.atLeastOneAtOrBelow40Rate,
  ...
  for (const domain of ANNUAL_AXIS_DOMAINS) {
    ...
    const dZ = ds.map((s) => s.effectiveZ);
    metrics[`domainMedian_${domain}`] = median(dScores);
    metrics[`domainPositiveEffectiveZRate_${domain}`] = rate(
      dZ.filter((v) => v > 0).length,
      dZ.length,
    );
    metrics[`domainNegativeEffectiveZRate_${domain}`] = rate(
      dZ.filter((v) => v < 0).length,
      dZ.length,
    );
...
export function evaluateV08HoldoutGates(samples: V08HoldoutSample[]): {
  metrics: Record<string, number>;
  gateResults: V08GateResult[];
  passedAllGates: boolean;
  blockers: string[];
} {
  const m = computeV08HoldoutMetrics(samples);
  const gates: V08GateResult[] = [
    g("allFinite", num(m, "allFinite"), 1, ">="),
    g("allScoresInRange", num(m, "allScoresInRange"), 1, ">="),
    g("unavailableRateMax", num(m, "unavailableRate"), 0.02, "<="),
    g("boundaryScoreRateMax", num(m, "boundaryScoreRate"), 0.04, "<="),
    g("tp4cSignedContributionExactZero", num(m, "tp4cSignedContributionMaxAbs"), 0, "=="),
    g("globalMedianScoreMin", num(m, "globalMedianScore"), 47, ">="),
    g("globalMedianScoreMax", num(m, "globalMedianScore"), 53, "<="),
    g("globalMeanScoreMin", num(m, "globalMeanScore"), 46, ">="),
    g("globalMeanScoreMax", num(m, "globalMeanScore"), 54, "<="),
    g("positiveEffectiveZRateMin", num(m, "positiveEffectiveZRate"), 0.35, ">="),
    g("positiveEffectiveZRateMax", num(m, "positiveEffectiveZRate"), 0.65, "<="),
    g("negativeEffectiveZRateMin", num(m, "negativeEffectiveZRate"), 0.35, ">="),
    g("negativeEffectiveZRateMax", num(m, "negativeEffectiveZRate"), 0.65, "<="),
    g("meanIntraYearSixAxisSdMin", num(m, "meanIntraYearSixAxisSd"), 11, ">="),
    g("medianIntraYearRangeMin", num(m, "medianIntraYearRange"), 30, ">="),
    ...
    g("atLeastOneAtOrBelow40RateMin", num(m, "atLeastOneAtOrBelow40Rate"), 0.6, ">="),
    ...
    g("maxAbsInterAxisCorrelationMax", num(m, "maxAbsInterAxisCorrelation"), 0.88, "<="),
  ];

  for (const domain of ANNUAL_AXIS_DOMAINS) {
    gates.push(g(`domainMedianMin_${domain}`, num(m, `domainMedian_${domain}`), 43, ">="));
    gates.push(g(`domainMedianMax_${domain}`, num(m, `domainMedian_${domain}`), 57, "<="));
    gates.push(
      g(
        `domainPositiveEffectiveZRateMin_${domain}`,
        num(m, `domainPositiveEffectiveZRate_${domain}`),
        0.25,
        ">=",
      ),
    );
    gates.push(
      g(
        `domainPositiveEffectiveZRateMax_${domain}`,
        num(m, `domainPositiveEffectiveZRate_${domain}`),
        0.75,
        "<=",
      ),
    ...
    gates.push(
      g(
        `domainNegativeEffectiveZRateMin_${domain}`,
        num(m, `domainNegativeEffectiveZRate_${domain}`),
        0.25,
        ">=",
      ),
    );
    gates.push(
      g(
        `domainNegativeEffectiveZRateMax_${domain}`,
        num(m, `domainNegativeEffectiveZRate_${domain}`),
        0.75,
        "<=",
      ),
    ...
    gates.push(
      g(`domainTwelveYearRangeMin_${domain}`, num(m, `domainTwelveYearRange_${domain}`), 12, ">="),
    );
    gates.push(
      g(`domainAdjacentDeltaMin_${domain}`, num(m, `domainAdjacentDelta_${domain}`), 2.5, ">="),
    );
  }
